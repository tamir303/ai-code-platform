...
async def _seed_session(title="Seeded Session"):
...
class TestListSessions:
    ...
    async def test_pagination_params(self, client):
        ...
        resp = await client.get("/api/v1/sessions?limit=10&offset=0")
        assert resp.status_code == 200
        assert len(resp.json()) == 1

        resp_offset = await client.get("/api/v1/sessions?limit=10&offset=1")
        assert resp_offset.status_code == 200
        assert resp_offset.json() == []

    async def test_invalid_pagination_limit(self, client):
        """GET /api/v1/sessions with invalid limit (> 100 or < 1) returns 422."""
        resp_too_high = await client.get("/api/v1/sessions?limit=101")
        assert resp_too_high.status_code == 422

        resp_too_low = await client.get("/api/v1/sessions?limit=0")
...
class TestGetSessionDetail:
    async def test_found(self, client):
        ...
        resp = await client.get(f"/api/v1/sessions/{TEST_SESSION_ID}")
    ...
    async def test_messages_pagination(self, client):
        ...
        resp = await client.get(f"/api/v1/sessions/{TEST_SESSION_ID}?limit=2&offset=1")
...
class TestDeleteSession:
    async def test_success(self, client):
        ...
        resp = await client.delete(f"/api/v1/sessions/{TEST_SESSION_ID}")

        assert resp.status_code == 204

        # Verify it's gone
        resp2 = await client.get(f"/api/v1/sessions/{TEST_SESSION_ID}")
